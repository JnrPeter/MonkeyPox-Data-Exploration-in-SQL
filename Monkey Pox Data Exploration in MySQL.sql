 ...
 -- Total number of confirmed and suspected cases worldwide
 
 With t1 as (SELECT 
    Country, COUNT(*) AS number_confirmed_cases FROM
    patient_info WHERE
    Status = 'confirmed'
GROUP BY 1
ORDER BY 2 ),
 ...
 SELECT 
    SUM(t1.number_confirmed_cases) AS confirmed_cases,
    SUM(t2.number_suspected_cases) AS suspected_cases
FROM
    t1
        JOIN
